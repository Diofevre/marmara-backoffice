...
import { addPromotion, AddPromotionPayload, updatePromotion } from '@/lib/services/promotionService';
  ...
  useEffect(() => {
    if (editPromotion) {
      setFormData({
        ...
        startDate: new Date(editPromotion.startDate).toISOString().split('T')[0],
                    ...
                    {filteredDishes.map(dish => (
                      <div 
                        ...
                        className={`p-3 flex items-center cursor-pointer hover:bg-[#FE724C]/20 ${
                          formData.platIds.includes(dish._id) ? 'bg-[#FE724C]/5' : ''
                        }`}
