...
const OrderTicket: React.FC<OrderTicketProps> = ({ order, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-[100mm] max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl">
        {/* Header Section */}
        <div className="bg-gradient-to-b from-gray-50 to-white px-6 pt-8 pb-6">
          <div className="text-center">
            <h1 className="text-3xl font-bold tracking-[0.25em] text-gray-900 mb-3">MARMARA SPRA</h1>
            <p className="text-lg font-medium text-gray-700">Order #{order.reference}</p>
            <p className="text-sm text-gray-500 mt-1">{format(new Date(order.date), 'dd/MM/yyyy HH:mm')}</p>
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-4 shadow-sm ring-1 ring-gray-100">
            <h2 className="font-semibold text-gray-900 uppercase text-sm mb-3">Customer Details</h2>
            {order.userId ? (
              <div className="space-y-1">
                <p className="font-medium text-gray-800">{order.userId.firstName} {order.userId.lastName}</p>
          ...
          <div className="space-y-4">
            <h2 className="font-semibold text-gray-900 uppercase text-sm">Order Items</h2>
            <div className="divide-y divide-gray-100">
              {order.items.map((item, index) => (
                <div key={index} className="py-3 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <span className="font-mono font-bold text-gray-900">{item.quantity}x</span>
                    <span className="text-gray-700">{item.platId?.name || item.packId?.name}</span>
                  </div>
          ...
          <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl p-4 text-white shadow-xl">
            <h2 className="text-gray-300 uppercase text-xs mb-1">Total Amount</h2>
            <p className="text-2xl font-bold">{order.amount.toFixed(2)}€</p>
          </div>

          {/* Footer */}
          <div className="text-center pt-4 border-t border-gray-100">
            <p className="text-gray-700 font-medium">Thank you for choosing Marmara SPRA!</p>
            <p className="text-xs text-gray-500 mt-2">{format(new Date(), 'dd/MM/yyyy HH:mm')}</p>
          </div>
